...
export class VulpMap {
	...
	public mappers: Map<string, [PromiseWithResolvers<MappedModule>, Mapper]> =
		new Map();
	...
	async checkModuleAsync(module: WebpackCacheObject) {
		...
		if (!factory) {
			throw Error(
				`Expected cached module ${module.id} to have a present factory in weboackFactories.`,
			);
		}
	...
	async registerMapperAndSearch(name: string, mapper: Mapper) {
		...
		if (this.webpack) {
			...
			for (const [id, cached] of Object.entries(webpack.webpackCache)) {
				...
				if (!factory) {
					throw Error(
						`Expected cached module ${id} to have a present factory in weboackFactories.`,
					);
				}
