...
vulpmap
	.registerMapperAndSearch("React/JsxModule", {
		map(found, builder) {
			builder.mapProperty(found);
		},
		filterType: FilterType.ExportsFilter,
		filter: new FullExportsFilterBuilder(
			utils.byPropsFilter("jsxs", "jsx", "Fragment"),
		)
			.retrieveRoot(true)
			.finish(),
	})
...
vulpmap
	.registerMapperAndSearch("React/React", {
		map(found, builder) {
			builder.mapProperty(found);
		},
		filterType: FilterType.ExportsFilter,
		filter: new FullExportsFilterBuilder(
			utils.byPropsFilter("useEffect", "useReducer", "useState", "useMemo"),
		)
			.retrieveRoot(false)
			.finish(),
	})
