					...
					processAst(ast, markDirty) {
						vulpatch.walk({
							...
							node(node) {
								...
								const keyRecognition = vulpatch.getProp(
									obj,
									"analyticsKey",
								)?.value;
								if (
									keyRecognition?.type !== "Literal" ||
									keyRecognition.value !== "user_settings"
								)
									return;
	...
	pluginsItem: () => {
		...
		const custom = createCustom("packages_button", {
			Component: () => {
				...
				return (
					<>
						{packageStore.isPending && (
							<Container>
								<h1 className={"paddedBottom"}>
									Packages enabled/disabled require a refresh of the Discord
									client to update its status.
								</h1>
							</Container>
						)}
						{[...packageStore.entries].map(([k, v]) => (
							<Package
								key={k}
								pack={v.package}
								status={v.toggled}
								toggle={() =>
									packageStore.setToggled(v.package.name, !v.toggled)
								}
							></Package>
						))}
					</>
...
function Package({ pack, status, toggle }: PackageProps) {
	return (
		<Container>
			<h1
				className={"halfPaddedBottom fc-prominent-text"}
			>{`${pack.name}:`}</h1>
