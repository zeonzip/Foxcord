...
export class PackageStore extends FoxcordStore {
	...
	private rawRegister(pack: PackageInterface, toggled = pack.core === true) {
		...
		this.packages.set(pack.name, {
			package: pack,
			toggled,
			loaded: toggled && (isStartup() || hotLoadable(pack)),
		});
		if (toggled) {
			if (isStartup()) {
...
export function startupLoadPackage(pack: PackageInterface) {
	...
	patches.regexPatches.push(
		...pluginPatches.filter((patch) => patch.type === PatchType.RegexPatch),
	);
	patches.astPatches.push(
		...pluginPatches.filter((patch) => patch.type === PatchType.AstPatch),
	);
